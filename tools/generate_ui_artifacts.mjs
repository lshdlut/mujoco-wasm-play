...
const bindingIndexPath = path.join(repoRoot, 'spec', 'ui_bindings_index.json');
const schemaPath = path.join(repoRoot, 'spec', 'ui_spec.schema.json');
const structsPath = path.join(repoRoot, 'core', 'viewer_structs.mjs');
const sharedPath = path.join(repoRoot, 'core', 'viewer_shared.mjs');
const defaultsPath = path.join(repoRoot, 'core', 'viewer_defaults.mjs');
const typesPath = path.join(repoRoot, 'core', 'viewer_state_types.ts');
...
function requireLeftPanelSection(sectionId) {
  const section = (spec.left_panel || []).find((entry) => entry?.section_id === sectionId);
  if (!section) {
    throw new Error(`Missing left_panel section: ${sectionId}`);
  }
  return section;
}

function extractSceneFlagNames() {
  const renderingSection = requireLeftPanelSection('rendering');
  const group = (renderingSection.trail_groups || []).find((entry) => entry?.group_id === 'rendering.opengl_flags');
  const entries = Array.isArray(group?.entries) ? group.entries : null;
  if (!entries || entries.length !== sceneFlagDefaults.length) {
    throw new Error('rendering.opengl_flags entries must match runtime.scene_flag_defaults length');
  }
  return entries.map((entry, idx) => {
    const name = String(entry?.name || `FLAG_${idx}`);
    return name
...
function extractViewerGroupDefaultMask() {
  const groupSection = requireLeftPanelSection('group');
  const mask = [];
  for (let idx = 0; idx < groupCount; idx += 1) {
    const item = (groupSection.items || []).find((entry) => entry?.item_id === `group.geom${idx}`);
    mask[idx] = !!item?.default;
  }
  return mask;
}

const sceneFlagNames = extractSceneFlagNames();
const sceneFlagIndices = Object.freeze(Object.fromEntries(sceneFlagNames.map((name, index) => [name, index])));
const viewerGroupDefaultMask = extractViewerGroupDefaultMask();
const viewerGroupDefaults = Object.freeze(Object.fromEntries(
  groupTypes.map((type) => [type, viewerGroupDefaultMask]),
));
...
function renderViewerDefaults() {
  const lines = [
    ...
    '',
    `export const VIEWER_GROUP_DEFAULTS = Object.freeze(${JSON.stringify(viewerGroupDefaults)});`,
    '',
    `export const SCENE_FLAG_DEFAULTS = Object.freeze(${JSON.stringify(sceneFlagDefaults)});`,
    `export const SCENE_FLAG_INDICES = Object.freeze(${JSON.stringify(sceneFlagIndices)});`,
    '',
