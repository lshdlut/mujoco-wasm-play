...
function parseJsLike(relPath, content) {
  ...
  if (m) {
      exports.push({ kind: 'export default', name: '', line: lineNo });
  }
...
function renderMarkdown({ language, generatedAt, files }) {
  const title = language === 'zh' ? '代码清单' : 'Code inventory';
  ...
  for (const group of groups) {
    ...
    for (const f of groupFiles) {
      ...
      if (f.exports.length) {
        out.push(language === 'zh' ? '- 导出：' : '- Exports:');
        for (const ex of f.exports) {
          const namePart = ex.name ? ` \`${ex.name}\`` : '';
          out.push(`  - L${ex.line}: ${ex.kind}${namePart}`);
        }
      } else {
        out.push(language === 'zh' ? '- 导出：无' : '- Exports: none');
      }
      if (f.declsFile.length) {
        out.push(language === 'zh' ? '- 文件级声明：' : '- File-scope declarations:');
        for (const d of f.declsFile) {
          out.push(`  - L${d.line}: ${d.kind} \`${d.name}\``);
        }
      } else {
        out.push(language === 'zh' ? '- 文件级声明：未发现' : '- File-scope declarations: none found');
      }
      if (f.declsNested.length) {
        out.push(language === 'zh' ? '- 嵌套声明：' : '- Nested declarations:');
        for (const d of f.declsNested) {
          out.push(`  - L${d.line}: ${d.kind} \`${d.name}\``);
        }
      } else {
        out.push(language === 'zh' ? '- 嵌套声明：无' : '- Nested declarations: none');
      }
      if (f.classMembers.length) {
        out.push(language === 'zh' ? '- 类成员：' : '- Heuristic class members:');
        for (const cm of f.classMembers) {
          out.push(`  - L${cm.line}: ${cm.kind} \`${cm.name}\``);
        }
      } else {
        out.push(language === 'zh' ? '- 类成员：无' : '- Heuristic class members: none');
      }
      out.push('');
    }
  }
