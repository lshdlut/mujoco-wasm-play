...
export function collectRenderAssetsFromModule(mod, handle) {
  ...
  if (ntendon > 0) {
    const nwrap = typeof mod._mjwf_model_nwrap === 'function'
      ? (mod._mjwf_model_nwrap(handle) | 0)
      : (typeof mod._mjwf_nwrap === 'function' ? (mod._mjwf_nwrap(handle) | 0) : 0);
...
export class MjSimLite {
  ...
  _tryHelperMakeFromXml(paths){
    ...
    for (const target of list){
      ...
      if (hasCcall){
        ...
        // Legacy-only fallback when ccall is unavailable.
  ...
  sceneGeomTransparentView(){ const m=this.mod; const h=this.h|0; const n=this.sceneNgeom()|0; if(!(n>0)) return null; const fn=m?._mjwf_scene_geoms_transparent_ptr; if (typeof fn!=='function') return null; const ptr=fn.call(m,h)|0; if(!(ptr>0)) return null; return heapViewI32(m, ptr, n); }
