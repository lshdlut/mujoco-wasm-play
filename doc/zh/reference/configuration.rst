...
大部分配置通过 URL 参数完成，见 :doc:`url_parameters`。本页面集中记录开发者向开关与调试 hook。
...
- ``nogeom=1``：设置初始的“隐藏全部 geometry”渲染开关。别名：``no_geom``、``no-geom``、``hideall``、``hide_all``。
...
- ``tmode=...``：透明排序模式。可用值包括 ``nosort``/``fast``、``bins``。其它值视作 strict。
...
- ``globalThis.PLAY_PLUGINS``: 插件 import specifier/URL 的数组。可替代 ``plugins=`` 查询参数。
...
- ``window.__viewerRenderer``: 渲染器辅助对象，例如 stats/context/overlay3d。
...
- ``window.__snapshot``: 若设为 ``true`` 或 ``1``，强制 WebGL ``preserveDrawingBuffer``，用于截图/调试。
