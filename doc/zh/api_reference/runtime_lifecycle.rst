...
- ``app/main.mjs``：主入口点。组装 UI、renderer、backend，并暴露插件 Host。

Worker：

- ``worker/physics.worker.mjs``：module Worker 入口点。负责 forge/WASM 加载、MuJoCo 步进，并发出 snapshots。
...
1. **HTML 加载** ``app/main.mjs``，以 ESM 的形式运行。
2. **消费 URL/全局配置**：

   - URL 辅助函数在 ``core/viewer_runtime.mjs``。包括 ``consumeViewerParams(...)``、
     ``buildWorkerUrl(...)`` 以及 strict/verbose 开关等。
3. **创建 backend**：
...
4. **Worker 内解析并加载 forge dist**：
   ...
   - Worker 解析 forge ``dist/<ver>/`` 基址，来源为 ``forgeBase=...`` 或本地回退。
...
- 将 snapshot 字段合并进 viewer store：
  ...
  - ``app/main.mjs`` 也会维护一个 ``latestSnapshot`` 便于快速访问，并为调试暴露
...
- Worker 步进
- 快照投递，自适应且由 worker 驱动
- UI 更新，为 DOM 工作提供节流 lanes

主入口点安装了一个小型 clock/订阅系统，提供：

- ``onSnapshot``：与 snapshot 对齐的工作，用于状态派生
- ``onFrame``：渲染帧屏障，用于 overlay commits 和逐帧动画
...
- ``renderer/pipeline.mjs`` 导出 ``createRendererManager(...)``：场景与渲染循环。
- ``renderer/controllers.mjs`` 导出：

  - ``createCameraController(...)``：相机交互与同步
  - ``createPickingController(...)``：picking 与 selection
- ``environment/environment.mjs`` 导出 ``createEnvironmentManager(...)``：sky 与 environment 管理。
