...
The plugin exports ``registerPlayPlugin(host)``. It may also use a default
export. The register function can return a disposer:
...
- snapshot streams: ``host.backend.subscribe(...)`` or ``host.clock.onSnapshot``

For the full contract, see :doc:`/reference/plugin_contract`.
